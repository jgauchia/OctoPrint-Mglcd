...
class NextionPlugin(octoprint.plugin.StartupPlugin,
	...
	def handshakeReceived(self):
		...
		self.nextionDisplay.nxWrite('home.t0Temp.txt="---"')
		self.nextionDisplay.nxWrite('home.t0Target.txt="---"')
		...
		if self.displayConnected:
			...
			if self.currentPage == 'home':
				...
				try:
					...
					if printTime > 60:	
						printTimeString = str(int(math.floor(printTime/3600)))+":"+str(int(math.floor(printTime/60)))+":"+str(int(math.fmod(printTime,60)))
					else:
				           if printTime > 9:
						printTimeString = "00:00:"+str(printTime)
				           else:
					        printTimeString = "00:00:0"+str(printTime)
					
					fileTimeString = self.currentPage + '.fileTime.txt="{}"'.format(printTimeString)

				except:	
					fileTimeString = self.currentPage + '.fileTime.txt="00:00:00"'
